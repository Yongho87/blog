var store = [{
        "title": "첫번째 글",
        "excerpt":"다시 온라인 상에 내 생각과 관련된 글을 작성하는 일을 시작해보려고 한다. 어떠한 플랫폼에 종속되지 않고 내 100%의 영향력 안에서 만들어지고 운영될 수 있는 블로그로 시작하려고 했는데 github page를 통해서 아주 쉽게 유지/보수/호스팅할 수 있어서 github 아니 마이크로소프트에 종속되기로. 아무튼 내가 평상시에 하는 생각중에 정리하거나 기억하고 싶은 사항들을 정리해놓는게 목적. 또는...","categories": ["jekyll","update"],
        "tags": [],
        "url": "/blog/jekyll/update/2021/02/14/%EC%B2%AB%EB%B2%88%EC%A7%B8-%EB%B8%94%EB%A1%9C%EA%B7%B8-%EA%B8%80.html",
        "teaser": null
      },{
        "title": "How to resolve expo eject could not locate a valid AppDelegate at root issue",
        "excerpt":"TL;DR There are two possible solutions depending on your case. delete android and ios folders before expo eject In app.json, change your expo.name field value to English value. a little longer version I encountered this weird issue and wasted about 5 hours on Saturday. There are few google search results...","categories": [],
        "tags": [],
        "url": "/blog/2021/02/14/expo-eject-ios-appdelegate-issue.html",
        "teaser": null
      }]
